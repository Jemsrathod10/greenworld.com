...
const Orders = () => {
  ...
  // ✅ ENHANCED: Comprehensive Image Resolver for all edge cases
  const resolveImageUrl = (item) => {
    // 1. Direct check in the order item (String)
    if (item.image && typeof item.image === 'string' && item.image.length > 5) return item.image;
    
    // 2. Check if product is populated (Object)
    const p = item.product;
    if (p && typeof p === 'object') {
      // 2a. Product images array (from your Product Model)
      if (p.images && Array.isArray(p.images) && p.images.length > 0) {
        const firstImg = p.images[0];
        return (typeof firstImg === 'object' ? (firstImg.url || firstImg.secure_url) : firstImg);
      }
      // 2b. Product single image string
      if (p.image && typeof p.image === 'string') return p.image;
    }

    // 3. Last Resort: Try to find any URL-like string in the object
    if (item.images && Array.isArray(item.images) && item.images.length > 0) {
        const fallbackImg = item.images[0];
        return (typeof fallbackImg === 'object' ? (fallbackImg.url || fallbackImg.secure_url) : fallbackImg);
    }

    // Final Fallback: UI Avatar
    return `https://ui-avatars.com/api/?name=${encodeURIComponent(item.name || 'P')}&background=79ac78&color=fff`;
  };
  ...
  const generateInvoice = async (order) => {
    ...
    const fullName = `${b.firstName || ''} ${b.lastName || ''}`.trim() || 'Customer';
  ...
  const styles = `
    .orders-container { max-width: 1000px; margin: 0 auto; padding: 4rem 1rem; font-family: 'Quicksand', sans-serif; }
    .page-title { font-family: 'Fraunces', serif; font-size: 2.5rem; text-align: center; margin-bottom: 3rem; }
    .order-card { background: white; border: 2px solid #5d6d36; border-radius: 16px; padding: 1.5rem; margin-bottom: 2rem; box-shadow: 8px 8px 0px rgba(93, 109, 54, 0.15); }
    ...
    .item-img { width: 55px; height: 55px; border-radius: 8px; object-fit: cover; border: 1px solid #ddd; background: #eee; }
    .order-footer { display: flex; justify-content: space-between; align-items: center; }
    .total-price { font-family: 'Fraunces', serif; font-size: 1.4rem; color: #5d6d36; }
    .btn-action { padding: 8px 16px; border-radius: 50px; font-weight: bold; cursor: pointer; display: inline-flex; align-items: center; gap: 6px; border: none; }
    .btn-invoice { background: #5d6d36; color: white; }
                  ...
                  {items.map((item, i) => (
                    <div key={i} className="item-row">
                      <img 
                        ...
                        alt={item.name} 
                        className="item-img" 
                        onError={(e) => { e.target.src = 'https://ui-avatars.com/api/?name=P&background=79ac78&color=fff'; }}
                      />
                      <div>
